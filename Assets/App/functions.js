...
async function fetchAndLoadDetailsOfBooks(bookId, query) {
  ...
  try {
    const detailsOfEachBookRes = await fetch(
      `https://openlibrary.org/api/books?bibkeys=OLID:${bookId}&jscmd=details&format=json`
    );
    if (!detailsOfEachBookRes.ok)
      throw new Error(
        "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
      );
    const detailsOfEachBookData = await detailsOfEachBookRes.json();

    for (const key in detailsOfEachBookData) {
      let bookPrice =
        detailsOfEachBookData[key].details.notes?.value === undefined
          ? detailsOfEachBookData[key].details.notes?.substring(5)
          : detailsOfEachBookData[key].details.notes?.value.substring(5);
      bookPrice = isNaN(Number(bookPrice))
        ? Number(bookId.slice(-4, -1))
        : bookPrice;
      let cardTextContent;
      let discountBadge;
      let salePrice;
      ...
      document.querySelector(`${query}`).insertAdjacentHTML(
        "beforeend",
        `
      ...
      return { ...detailsOfEachBookData[key], bookPrice, salePrice, bookId };
    }
  } catch (error) {
...
async function loadDataInBoxesOnIndexPage() {
  ...
  try {
    const psychologyRes = await fetch(
      "https://openlibrary.org/subjects/روانشناسی.json?limit=15"
    );
    if (!psychologyRes.ok)
      throw new Error(
        "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
    ...
    const data = await psychologyRes.json();
    psychologyData = await Promise.all(
      data.works
        .filter((book) => !!book?.cover_edition_key)
        .map((book) =>
          fetchAndLoadDetailsOfBooks(
            book.cover_edition_key,
            "#navPsychology .tab-pane-container"
          )
        )
    );
  } catch (error) {
  ...
  try {
    const poetryRes = await fetch(
      "https://openlibrary.org/subjects/شعر.json?offset=6&limit=15"
    );
    if (!poetryRes.ok)
      throw new Error(
        "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
    ...
    const data = await poetryRes.json();
    poetryData = await Promise.all(
      data.works
        .filter((book) => !!book?.cover_edition_key)
        .map((book) =>
          fetchAndLoadDetailsOfBooks(
            book.cover_edition_key,
            "#navPoetry .tab-pane-container"
          )
        )
    );
  } catch (error) {
  ...
  try {
    const foreignRes = await fetch(
      "https://openlibrary.org/subjects/fantasy.json?language=eng&limit=15&sort=random"
    );
    if (!foreignRes.ok)
      throw new Error(
        "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
    ...
    foreignData = await Promise.all(
      data.works
        .filter((book) => !!book?.cover_edition_key)
        .map((book) =>
          fetchAndLoadDetailsOfBooks(
            book.cover_edition_key,
            "#navForeign .tab-pane-container"
          )
        )
    );
  } catch (error) {
  ...
  try {
    const fictionRes = await fetch(
      "https://openlibrary.org/subjects/ادبیات_داستانی.json?limit=15"
    );
    if (!fictionRes.ok)
      throw new Error(
        "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
    ...
    const data = await fictionRes.json();
    fictionData = await Promise.all(
      data.works
        .filter((book) => !!!book?.cover_edition_key)
        .map((book) =>
          fetchAndLoadDetailsOfBooks(
            book.cover_edition_key,
            "#navFiction .tab-pane-container"
          )
        )
    );
  } catch (error) {
  ...
  allBooksData.forEach((book) => {
    ...
    if (book?.bookPrice > 150) {
      //the href
      let href = `product.html?id=${book?.bookId}`;
      //load data on the page
      document
        .querySelector(".sale-box .recommend-box-cards")
        .insertAdjacentHTML(
          "beforeend",
          `
  ...
  while (loadedBook < 15) {
    ...
    if (!randomIndexArray.includes(randomIndex)) {
      //if the book with this index hasn't already been loaded, add the book
      randomIndexArray.push(randomIndex);
      let book = allBooksData[randomIndex];
      //for accessing the price of each book, we need tp fetch the more detaild data of each book
      document.querySelector(".popular-box .book-loader").style.display =
        "none"; //hiding book loader
      let cardTextContent;
      let discountBadge;
      //if the price of the book was more than 150, add the sale price and the discount badge
      if (book?.bookPrice > 150) {
      ...
      //load data on the page
      document
        .querySelector(".popular-box .recommend-box-cards")
        .insertAdjacentHTML(
          "beforeend",
          `
...
async function loadDataOnCategoriesPage(cParam) {
  ...
  switch (cParam) {
    case "foreign":
      ...
      try {
        res = await fetch(
          `https://openlibrary.org/subjects/fantasy.json?language=eng&limit=70`
        );
        if (!res.ok)
          throw new Error(
            "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
    ...
    case "psychology":
      ...
      try {
        res = await fetch(
          `https://openlibrary.org/subjects/${category}.json?limit=70`
        );
        if (!res.ok)
          throw new Error(
            "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
    ...
    case "poetry":
      ...
      try {
        res = await fetch(
          `https://openlibrary.org/subjects/${category}.json?offset=6&limit=70`
        );
        if (!res.ok)
          throw new Error(
            "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
  ...
  let fetchPromises = data.works.map(async (book) => {
    try {
      const detailsOfEachBookRes = await fetch(
        `https://openlibrary.org/api/books?bibkeys=OLID:${book.cover_edition_key}&jscmd=details&format=json`
      );
      if (!detailsOfEachBookRes.ok)
        throw new Error(
          "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
      ...
      for (const key in detailsOfEachBookData) {
        let publisherName =
          detailsOfEachBookData[key].details.publishers != undefined
            ? detailsOfEachBookData[key].details.publishers[0]
            : "";
        let realPrice =
          detailsOfEachBookData[key].details.notes?.value === undefined
            ? detailsOfEachBookData[key].details.notes?.substring(5)
        ...
        if (realPrice > 150) salePrice = Math.floor(realPrice * 0.7); //their price with discount
        allBooks.push({
          id: book.cover_edition_key,
          title: detailsOfEachBookData[key].details.title,
          realPrice: Number(realPrice),
          salePrice: Number(salePrice),
          publisherName,
          createdDate: detailsOfEachBookData[key].details.created.value,
        });
      }
    } catch (error) {
      console.error("با یک ارور غیر منتظره مواجه شدیم:", error);
  ...
  allBooksSortedNewest = [...allBooks].sort(
    (a, b) => new Date(b.createdDate) - new Date(a.createdDate)
  ); //sort all books from the newest one to the oldest one
}
...
function sortAndFilterBooks(
  ...
  pageNumber,
  makePaginationBtns,
  addRemovingEventsToAppliedFilter
) {
  ...
  if (sortFilter === "newest") {
    allBooksSortedNewest.forEach((book) => {
      ...
      if (publisherFilter === "all") {
        if (
          book.salePrice * 1000 >= minPriceRange &&
          book.salePrice * 1000 <= maxPriceRange
        )
        ...
        if (
          book.salePrice * 1000 >= minPriceRange &&
          book.salePrice * 1000 <= maxPriceRange &&
          book.publisherName === publisherFilter
        )
          allFilteredBooks.push(book);
      }
    });
  ...
  setTimeout(() => {
    ...
    if (allFilteredBooks.length === 0) {
      ...
      for (let i = (pageNumber - 1) * 12; i < pageNumber * 12; i++) {
        if (i > allFilteredBooks.length - 1) break;
        let cardTextContent;
        let discountBadge;
        //if the price of the book was more than 150, add the sale price and the discount badge
    ...
    publishers.forEach((pub) => {
      ...
      liElem.addEventListener("click", (event) => {
        document.querySelectorAll(".pub-list li").forEach((li) => {
          ...
          if (li.innerHTML === liElem.innerHTML) li.classList.add("selected");

          publisherFilter = liElem.innerHTML;
        });
        document
          .querySelectorAll(".applied-filters .filters")
          .forEach((box) => {
        ...
        document
          .querySelectorAll(".applied-filters .publisher-filter i.fa-close")
          .forEach((closeIcon) => {
  ...
  document
    .querySelectorAll(".applied-filters .paginationInfo")
    .forEach((pi) => {
      pi.querySelector(".current-page").innerHTML =
        allFilteredBooks.length === 0 ? 0 : pageNumber;
...
function addCartItems() {
  ...
  document
    .querySelectorAll(".cart-icon .badge")
    .forEach((cartIcon) => (cartIcon.innerHTML = orderCount));
  ...
  document.querySelectorAll(".add-to-cart-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      let sameItemIndex = cartItems.findIndex(
        (item) => item.olid === btn.parentElement.dataset.olid
      );
      if (sameItemIndex === -1)
...
async function loadDataOnProductPage(bookId) {
  try {
    ...
    for (const key in bookData) {
      ...
      let subject;
      let allSubjects = [
        { title: "روانشناسی", href: "psycology" },
        { title: "شعر", href: "poetry" },
        { title: "ادبیات داستانی", href: "fiction" },
        { title: "آموزشی", href: "education" },
      ];
      subject = allSubjects.find(
        (s) => s.title === bookData[key].subjects[0].name
      );
      ...
      document
        .querySelector(".breadcrumb .subject")
        .setAttribute("href", `categories.html?c=${subject.href}`);
      document.querySelector(".breadcrumb .active").innerHTML =
        bookData[key].title;
      document.getElementById("bookTitle").innerHTML = bookData[key].title;
      let authorName =
        bookData[key].authors === undefined
          ? "ناشناس"
          : bookData[key].authors[0].name;
      document.getElementById("authorName").innerHTML = authorName;
      document
        .getElementById("bookPic")
        .setAttribute("src", bookData[key].cover.large);
      let isbn =
        bookData[key].identifiers.isbn_13 === undefined
          ? "9085895728136"
          : bookData[key].identifiers.isbn_13[0];
      document.getElementById("ISBN").innerHTML = isbn;
      document.getElementById("bookId").innerHTML = bookId;
      document.getElementById("publisher").innerHTML =
        bookData[key].publishers[0].name;
      document.getElementById("pageCount").innerHTML =
        bookData[key].number_of_pages;
      document.getElementById("publishDate").innerHTML =
        bookData[key].publish_date;

      let priceElementContent;

      let bookPrice =
        bookData[key].notes?.value === undefined
          ? bookData[key].notes?.substring(5)
          : bookData[key].notes?.value.substring(5);
      bookPrice = isNaN(Number(bookPrice))
        ? Number(bookId.slice(-4, -1))
        : bookPrice;
      if (bookPrice > 150) {
        document.getElementById("badgeContainer").insertAdjacentHTML(
          "beforeend",
          `
      ...
      document
        ...
        .insertAdjacentHTML("beforeend", priceElementContent);

      //load data in recommmendedBookBox
      let recommendedRes;
      if (subject.title === "ادبیات خارجی")
        recommendedRes = await fetch(
          `https://openlibrary.org/subjects/fantasy.json?sort=random`
        );
      else if (subject.title === "ادبیات داستانی")
        recommendedRes = await fetch(
          `https://openlibrary.org/subjects/ادبیات_داستانی.json?sort=random`
        );
      else
        recommendedRes = await fetch(
          `https://openlibrary.org/subjects/${subject.title}.json?sort=random`
        );
      let recommendedData = await recommendedRes.json();

      let fetchPromises = recommendedData.works.map(async (book) => {
        await fetchAndLoadDetailsOfBooks(
          book.cover_edition_key,
          "#recommendedBooksBox"
        );
      });
    ...
    for (const key in bookDetailsData) {
      let editionNum =
        bookDetailsData[key].details.edition_name === undefined
          ? 1
          : bookDetailsData[key].details.edition_name;
      document.getElementById("editionNum").innerHTML = editionNum;
      let physicalCover =
        bookDetailsData[key].details.physical_format === undefined
          ? "شومیز"
          : bookDetailsData[key].details.physical_format;
      document.getElementById("physicalCover").innerHTML = physicalCover;
      if (
        bookDetailsData[key].details.contributors &&
        bookDetailsData[key].details.contributors[0].role === "Translator"
      )
        document.querySelector(".book-info .part1").insertAdjacentHTML(
          "afterbegin",
          `
      ...
      if (bookDetailsData[key].details.description === undefined)
        document.getElementById("aboutThisBookText").innerHTML =
          "توضیحی برای این کتاب درج نشده است.";
      else if (
        bookDetailsData[key].details.description &&
        bookDetailsData[key].details.description.value === undefined
      )
        document.getElementById("aboutThisBookText").innerHTML =
          bookDetailsData[key].details.description;
      else if (
        bookDetailsData[key].details.description ||
        bookDetailsData[key].details.description.value
      )
        document.getElementById("aboutThisBookText").innerHTML =
          bookDetailsData[key].details.description.value;
    }
...
async function loadDataOnShoppingCartPage() {
  addCartItems();
  let cartTableDesktopScreen = document.querySelector(
    "#cartTableDesktopScreen tbody"
  );
  ...
  let fetchPromises = cartItems.map(async (item) => {
    try {
      const bookRes = await fetch(
        `https://openlibrary.org/api/books?bibkeys=OLID:${item.olid}&jscmd=data&format=json`
      );
      if (!bookRes.ok)
        throw new Error(
          "در حال حاضر سرور قادر به پاسخگویی نمی‌باشد. لطفاٌ بعدا امتحان کنید."
      ...
      for (const key in bookData) {
        ...
        cartTableDesktopScreen.insertAdjacentHTML(
          "beforeend",
          `
      ...
      `
        ...
        cartTableMobileScreen.insertAdjacentHTML(
          "beforeend",
          `
  ...
  Promise.all(fetchPromises).then(() => {
    ...
    document.querySelectorAll(".quantity-minus").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        document.querySelectorAll(".quantity-input").forEach((input) => {
          //changing the value of quantity-input in both mobile and desktop screens
          if (
            input.dataset.olid === e.target.previousElementSibling.dataset.olid
          ) {
            newQuantityValue =
              Number(e.target.previousElementSibling.value) - 1;
            if (newQuantityValue <= 0) newQuantityValue = 1;
            input.value = newQuantityValue;
          }
        });
        //changing the total price of the item in both mobile and desktop screens
        ...
        singlePrice = Number(singlePrice.substring(0, singlePrice.length - 4));
        let totalPrice =
          singlePrice * Number(e.target.previousElementSibling.value);
        document.querySelectorAll(".cart-table .total-price").forEach((tp) => {
          if (
    ...
    document.querySelectorAll(".remove-cart-item-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        let cartItems = JSON.parse(localStorage.getItem("cartItems"));
        let removedItemIndex = cartItems.findIndex(
          (item) =>
            item.olid === e.target.parentElement.parentElement.dataset.olid
        );
        ...
        document.querySelectorAll(".cart-table tr").forEach((tr) => {
          if (
            tr.dataset.olid ===
            e.target.parentElement.parentElement.dataset.olid
          ) {
            tr.remove();
          }
        });
...
function calculateTheCartBill() {
  ...
  document
    .querySelectorAll("#cartTableDesktopScreen .real-price")
    .forEach((price) => {
      let singleRealPrice = Number(
        price.innerHTML.substring(0, price.innerHTML.length - 4)
      );
      let orderCount = Number(
        price.parentElement.parentElement.parentElement.parentElement.querySelector(
          ".quantity-input"
        ).value
      );
      ...
      if (
        ...
        let singleSalePrice = Number(
          price.nextElementSibling
            .querySelector("span")
            .innerHTML.substring(
              0,
              price.nextElementSibling.querySelector("span").innerHTML.length -
                4
            )
        );
  ...
  document
    .querySelectorAll("#cartTableDesktopScreen .total-price")
    .forEach((price) => {
...
export {
  ...
  loadDataOnCategoriesPage,
  sortAndFilterBooks,
  loadDataOnProductPage,
